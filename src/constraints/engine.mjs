...
import { Coincident, Distance, Horizontal, Vertical, Parallel, Perpendicular, Equal } from './constraints.mjs';
...
export class ConstraintEngine {
  ...
  rebuild() {
    ...
    for (const [name, shape] of this.renderer.shapes.entries()) {
      ...
      // Universal bounding-box anchors for ALL shapes
      try {
        const b = this.renderer.transformManager.calculateBounds(shape);
        if (b && Number.isFinite(b.width) && Number.isFinite(b.height)) {
          const hx = b.width/2, hy = b.height/2;
  ...
  _solveConstraint(type, a, b, extra = {}, fixedShapeName = null) {
    ...
    if (type === 'coincident') {
      ...
      eqs = c.getEqs(ax,ay,bx,by);
    } else if (type === 'parallel' || type === 'perpendicular' || type === 'equal') {
      // Prefer the user-picked anchors as the first endpoints; derive complementary endpoints
      const pickLine = (shapeName, preferred) => {
        const anchors = this.getAnchorsForShape(shapeName) || [];
        const has = key => anchors.find(a => a.key === key);
        const complement = (key) => {
          const map = {
            rect_ml: 'rect_mr', rect_mr: 'rect_ml', rect_mt: 'rect_mb', rect_mb: 'rect_mt',
            rect_tl: 'rect_br', rect_tr: 'rect_bl', rect_bl: 'rect_tr', rect_br: 'rect_tl',
            circ_w: 'circ_e', circ_e: 'circ_w', circ_n: 'circ_s', circ_s: 'circ_n',
            ellipse_w: 'ellipse_e', ellipse_e: 'ellipse_w', ellipse_n: 'ellipse_s', ellipse_s: 'ellipse_n',
            tri_bl: 'tri_br', tri_br: 'tri_bl', tri_mb: 'tri_apex', tri_apex: 'tri_mb',
            arrow_tail: 'arrow_tip', arrow_tip: 'arrow_tail'
          };
          const c = map[key];
          if (c && has(c)) return c;
          if (has('rect_mr') && key !== 'rect_mr') return 'rect_mr';
          if (has('rect_ml') && key !== 'rect_ml') return 'rect_ml';
          if (has('circ_e') && key !== 'circ_e') return 'circ_e';
          if (has('circ_w') && key !== 'circ_w') return 'circ_w';
          return 'center';
        };
        const a1 = preferred && has(preferred) ? preferred : (has('rect_ml') ? 'rect_ml' : (has('circ_w') ? 'circ_w' : 'center'));
        const a2 = complement(a1);
        return { a1, a2 };
      };

      const L1 = pickLine(a.shape, a.anchor);
      const L2 = pickLine(b.shape, b.anchor);

      const idA1 = sym(`${L1.a1}_${a.shape}`);
      const idA2 = sym(`${L1.a2}_${a.shape}`);
      const idB1 = sym(`${L2.a1}_${b.shape}`);
      const idB2 = sym(`${L2.a2}_${b.shape}`);

      const xA1 = `x${idA1}`, yA1 = `y${idA1}`;
      const xA2 = `x${idA2}`, yA2 = `y${idA2}`;
      const xB1 = `x${idB1}`, yB1 = `y${idB1}`;
      const xB2 = `x${idB2}`, yB2 = `y${idB2}`;

      if (type === 'parallel') {
        c = new Parallel({id:idA1},{id:idA2},{id:idB1},{id:idB2});
        eqs = c.getEqs(xA1,yA1,xA2,yA2,xB1,yB1,xB2,yB2);
      } else if (type === 'perpendicular') {
        c = new Perpendicular({id:idA1},{id:idA2},{id:idB1},{id:idB2});
        eqs = c.getEqs(xA1,yA1,xA2,yA2,xB1,yB1,xB2,yB2);
      } else {
        c = new Equal({id:idA1},{id:idA2},{id:idB1},{id:idB2});
        eqs = c.getEqs(xA1,yA1,xA2,yA2,xB1,yB1,xB2,yB2);
      }
      // Include all endpoints in variables set
      idsToMove = [idA1, idA2, idB1, idB2];
      const vExtra = this._varsFor([idA1,idA2,idB1,idB2]);
      Object.assign(vars, vExtra);
    } else {
  ...
  addParallel(a, b) {
    this._solveConstraint('parallel', a, b);
    const def = this._store({ type:'parallel', a, b });
    return def;
  }

  addPerpendicular(a, b) {
    this._solveConstraint('perpendicular', a, b);
    const def = this._store({ type:'perpendicular', a, b });
    return def;
  }

  addEqual(a, b) {
    this._solveConstraint('equal', a, b);
    const def = this._store({ type:'equal', a, b });
    return def;
  }
  ...
  getConstraintList() {
    return this.constraints.map(c => {
      ...
      if (c.type === 'parallel' || c.type === 'perpendicular' || c.type === 'equal') {
        const Name = c.type.charAt(0).toUpperCase() + c.type.slice(1);
        return { id:c.id, label:`${Name}  ${describeAB()}` };
      }
