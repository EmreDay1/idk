export function setupConstraintsMenu({ renderer, constraintEngine, displayErrors }) {
  ...
  const E_t = label('Parallel / Perpendicular / Equal');
  const E_sa = sel(), E_sb = sel();
  const E_aa = sel(), E_ab = sel();
  E_sa.addEventListener('change', () => fillAnchors(E_aa, E_sa.value));
  E_sb.addEventListener('change', () => fillAnchors(E_ab, E_sb.value));
  const E_row = row2(btnFull('Parallel'), btnFull('Perpendicular'));
  const E_equal = btnFull('Equal Length');
  E_row.children[0].addEventListener('click', () => {
    try {
      if (!E_sa.value || !E_sb.value || !E_aa.value || !E_ab.value) return;
      const def = constraintEngine.addParallel(
        { shape:E_sa.value, anchor:E_aa.value },
        { shape:E_sb.value, anchor:E_ab.value }
      );
      window.__pendingConstraints = window.__pendingConstraints || []; window.__pendingConstraints.push({ type:'parallel', a:{shape:E_sa.value, anchor:E_aa.value}, b:{shape:E_sb.value, anchor:E_ab.value} });
      window.updateCodeFromConstraints && window.updateCodeFromConstraints();
      window.updateConstraintsMenu && window.updateConstraintsMenu(constraintEngine.getConstraintList());
    } catch(e){ err(e); }
  });
  E_row.children[1].addEventListener('click', () => {
    try {
      if (!E_sa.value || !E_sb.value || !E_aa.value || !E_ab.value) return;
      const def = constraintEngine.addPerpendicular(
        { shape:E_sa.value, anchor:E_aa.value },
        { shape:E_sb.value, anchor:E_ab.value }
      );
      window.__pendingConstraints = window.__pendingConstraints || []; window.__pendingConstraints.push({ type:'perpendicular', a:{shape:E_sa.value, anchor:E_aa.value}, b:{shape:E_sb.value, anchor:E_ab.value} });
      window.updateCodeFromConstraints && window.updateCodeFromConstraints();
      window.updateConstraintsMenu && window.updateConstraintsMenu(constraintEngine.getConstraintList());
    } catch(e){ err(e); }
  });
  E_equal.addEventListener('click', () => {
    try {
      if (!E_sa.value || !E_sb.value || !E_aa.value || !E_ab.value) return;
      const def = constraintEngine.addEqual(
        { shape:E_sa.value, anchor:E_aa.value },
        { shape:E_sb.value, anchor:E_ab.value }
      );
      window.__pendingConstraints = window.__pendingConstraints || []; window.__pendingConstraints.push({ type:'equal', a:{shape:E_sa.value, anchor:E_aa.value}, b:{shape:E_sb.value, anchor:E_ab.value} });
      window.updateCodeFromConstraints && window.updateCodeFromConstraints();
      window.updateConstraintsMenu && window.updateConstraintsMenu(constraintEngine.getConstraintList());
    } catch(e){ err(e); }
  });
  pop.appendChild(hr());
  pop.appendChild(E_t);  pop.appendChild(row2(E_sa, E_sb));  pop.appendChild(row2(E_aa, E_ab)); pop.appendChild(E_row); pop.appendChild(E_equal);
  pop.appendChild(hr());
